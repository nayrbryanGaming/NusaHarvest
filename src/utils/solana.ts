...
export async function isProtocolProgramDeployed(force = false): Promise<boolean> {
  ...
  const programAccount = await withTimeout(
...
export async function createPolicyTransaction(
  ...
  // 1. Derive PDAs — seeds must match contract: [b"policy", farmer_pubkey, policy_id]
  const [policyPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('policy'), farmer.toBuffer(), Buffer.from(policyData.policyId)],
    programId
  )
...
/**
 * Fetch PoolState PDA data (Total Value Locked and Active Policies)
 * Returns hardcoded fallback if RPC call fails
 */
export async function fetchPoolStateMetrics(): Promise<{ totalTvl: string; activePolicies: number }> {
  const connection = new Connection(RPC_URL, 'confirmed')
  try {
    const programId = getProgramId()
    
    // Derive PoolState PDA: ["pool_state"]
    const [poolStatePda] = PublicKey.findProgramAddressSync(
      [Buffer.from('pool_state')],
      programId
    )
    
    // Fetch account data with timeout
    ...
    // Parse Borsh data (simplified deserialization)
    // PoolState structure (from contract):
    // - admin: 32 bytes (Pubkey)
    // - treasury: 32 bytes (Pubkey)
    // - total_tvl_usdc: 8 bytes (u64)
    // - total_reserve_usdc: 8 bytes (u64)
    // - total_loans_active_usdc: 8 bytes (u64)
    // - active_policies_count: 8 bytes (u64)
    // - protocol_fee_bps: 2 bytes (u16)
    // - initialized_at: 8 bytes (i64)
    // - bump: 1 byte (u8)
    
    const data = accountInfo.data
    
    // Skip discriminator (8 bytes at start for Anchor)
    let offset = 8
    
    // Skip admin pubkey (32 bytes)
    offset += 32
    
    // Skip treasury pubkey (32 bytes)
    offset += 32
    
    // Read total_tvl_usdc (u64 at offset 72, little-endian)
    const totalTvlBigInt = data.readBigUInt64LE(offset)
    const totalTvlUsdc = Number(totalTvlBigInt) / 1_000_000 // Convert from lamports (6 decimals)
    offset += 8
    
    // Skip total_reserve_usdc (u64)
    offset += 8
    
    // Skip total_loans_active_usdc (u64)
    offset += 8
    
    // Read active_policies_count (u64)
    const activePoliciesCount = Number(data.readBigUInt64LE(offset))
    
    const tvlFormatted = totalTvlUsdc > 0 ? `$${totalTvlUsdc.toLocaleString('en-US', { maximumFractionDigits: 2 })} USDC` : '$0 USDC (Devnet)'
