...
export async function isProtocolProgramDeployed(force = false): Promise<boolean> {
  ...
  const rpcCheck = async (): Promise<boolean> => {
    ...
    return Boolean(programAccount?.executable)
  }
  ...
  try {
    const deployed = await Promise.race([rpcCheck(), timeout])
    cachedProgramCheck = { checkedAt: Date.now(), deployed }
    return deployed
  } catch {
    // RPC timeout or network error — resolve false so UI exits loading state
    // Use a short TTL so next navigation retries
    cachedProgramCheck = { checkedAt: Date.now() - PROGRAM_CHECK_TTL_MS + 10_000, deployed: false }
    return false
  }
}
