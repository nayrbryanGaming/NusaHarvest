...
import { Leaf, Shield, Zap, CloudRain, ChevronRight, Activity, Globe, ArrowUpRight, Lock, Cpu, Database } from 'lucide-react'
...
import { fetchPoolState, type PoolStateData } from '../utils/onchain'
...
const FEATURES = [
  {
    ...
    desc: 'Data curah hujan via Open-Meteo API (185+ stasiun Indonesia). Diverifikasi on-chain setiap 6 jam.'
  },
  {
...
export default function HomePage() {
  ...
  const [poolData, setPoolData] = useState<PoolStateData | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    fetchPoolState().then(d => { if (!cancelled) setPoolData(d) })
    return () => { cancelled = true }
  }, [])

  const tvlDisplay = poolData
    ? `$${poolData.totalTvlUsdc.toLocaleString('en-US', { maximumFractionDigits: 0 })} USDC`
    : 'Pre-Launch Phase'

  const policiesDisplay = poolData
    ? poolData.activePoliciesCount.toLocaleString('id-ID')
    : 'Target: 500'

  const STATS = [
    { label: 'Total Value Locked', value: tvlDisplay,      icon: Database,  accent: 'emerald' },
    { label: 'Active Policies',    value: policiesDisplay,  icon: Shield,    accent: 'teal'    },
    { label: 'Settlement',         value: '< 6 Jam',        icon: Zap,       accent: 'amber'   },
    { label: 'Pilot Region',       value: 'Jawa Tengah',    icon: Globe,     accent: 'indigo'  },
  ]
  ...
  return (
    <main className="min-h-screen relative overflow-hidden">
      ...
      {/* Header rendered globally via layout.tsx */}

      {/* ── Hero ────────────────────────────────────── */}
      <section className="relative pt-[calc(52px+4rem)] pb-16 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        {/* Background grid */}
        ...
        <motion.h1
          {...FV(0.08)}
          className="font-display text-5xl md:text-7xl lg:text-8xl text-white mb-5 tracking-tight leading-[1.06]"
        >
          Asuransi panen<br className="hidden md:block" />
          <em className="text-emerald-400"> cair dalam 6 jam.</em>
          <br className="hidden md:block" />Bukan 6 bulan.
        </motion.h1>
